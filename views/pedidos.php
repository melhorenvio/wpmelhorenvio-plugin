...
<div id="app">
    ...
    <div class="content">
        <div class="data-client">
            <div class="data-client__item -profile">
                ...
                <div>
                    ...
                    <img src="<?= plugins_url("assets/img/bgpdr.png", __DIR__) ?>" v-if="!user_info.thumbnail"
                         width="100px">
                    <div class="about">
                ...
                <div>
                    ...
                    <div>
                        <ul>
                            ...
                            <li><p>{{endereco.district}} - {{ endereco.city.city}} /
                                    {{endereco.city.state.state_abbr}}</p></li>
            ...
            <div class="data-client__item -options">
                ...
                <div>
                    <ul>
                        <li><p><span>Aviso de Recebimento:</span> <span class="circle"
                                                                        :class="{'true' : opcionais.AR}"></span></p>
                        </li>
                        <li><p><span>Disponível para o cliente:</span> <span class="circle"
                                                                             :class="{'true' : opcionais.CF}"></span>
                            </p></li>
                        <li><p><span>Dias extras:</span> <span>{{opcionais.DE}}</span></p></li>
                        <li><p><span>Mão Própria:</span> <span class="circle" :class="{'true' : opcionais.MP}"></span>
                            </p></li>
                        <li><p><span>Porcentagem de lucro</span> {{opcionais.PL}}%</p></li>
                        <li><p><span>Valor Declarado:</span> <span class="circle"
                                                                   :class="{'true' : opcionais.VD}"></span></p></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="link-box"><a
                    href="<?= get_admin_url(get_current_blog_id(), "/admin.php?page=wpmelhorenvio_melhor-envio-config") ?>">Editar
                configurações</a></div>

        <div v-if="pedidos.length < 1" class="wpme_nothing">
            <template v-if="!finished">
                <h3>
                    <svg class="ico" width="150" height="150" viewBox="0 0 150 150" xmlns="http://www.w3.org/2000/svg"
                         stroke="#3598dc ">
            ...
            <table>
                ...
                <tr class="action-line">
                    <td colspan="7">
                        ...
                        <a href="javascript:;" class="btn comprar-hard" @click.prevent="addManyToCart()"> Adicionar ao
                            Carrinho </a>
                        <a href="javascript:;" class="btn melhorenvio" @click.prevent="openMultiplePaymentSelector()">
                            Pagar </a>
                ...
                <tr v-for="(pedido, i) in pedidos_page" v-if="updated || !updated">
                    ...
                    <td>
                        <ul>
                            ...
                            <li>{{pedido.shipping.address_1}} {{pedido.shipping.address_2}} -
                                {{pedido.shipping.postcode}}
                            </li>
                            <li>{{pedido.shipping.neighborhood}} - {{pedido.shipping.city}}/{{pedido.shipping.state}}
                            </li>
                        </ul>
                    </td>
                    <td>
                        <template v-if="pedido.cotacoes.length > 0">
                            <span v-if="pedido.bought_tracking" v-for="cotacao in pedido.cotacoes"><template
                                        v-if="(cotacao.id == pedido.bought_tracking)">{{cotacao.company.name}} {{cotacao.name}} |  {{cotacao.delivery_time}}  dia<template
                                            v-if="cotacao.delivery_time > 1">s</template> | {{cotacao.currency}} {{cotacao.price}}</template></span>

                            <select class="select" v-model="selected_shipment[i]" v-if="!pedido.bought_tracking">
                                <option v-for="cotacao in pedido.cotacoes"
                                        v-if="(! cotacao.error )"
                                        :class="{'selected': Array.isArray(pedido.shipping_lines) && pedido.shipping_lines[0].method_id == 'wpmelhorenvio_'.concat(cotacao.company.name).concat('_').concat(cotacao.name)}"
                                        :value="cotacao.id"
                                >
                        ...
                        <template v-else>
                            <a href="javascript:;" class="btn comprar" @click.prevent="getCotations(pedido.id,i)">Calcular
                                Cotação</a>
                        </template>
                    </td>
                    <td>
                    ...
                    <td>
                        <template v-if="pedido.cotacoes.length > 0">
                            <template
                                    v-if="pedido.status != 'cart' && pedido.status != 'paid' && pedido.status != 'printed' && pedido.status != 'waiting'">
                                <a href="javascript:;" class="btn comprar" @click.prevent="addToCart(i)"> Adicionar ao
                                    carrinho </a>
                            </template>
                            <template v-if="pedido.status == 'cart'">
                                <a href="javascript:;" class="btn melhorenvio"
                                   @click.prevent="openSinglePaymentSelector(pedido.tracking_code)"> Pagar </a>
                                <a href="javascript:;" class="btn cancelar"
                                   @click.prevent="removeFromCart(i,pedido.tracking_code)"> Remover </a>
                            </template>
                            <template v-if="pedido.status == 'paid'">
                                <a href="javascript:;" class="btn imprimir"
                                   @click.prevent="printTicket(pedido.tracking_code)"> Imprimir </a>
                                <a href="javascript:;" class="btn cancelar"
                                   @click.prevent="openCancelTicketConfirmer(pedido.tracking_code)"> Cancelar </a>
                            </template>
                            <template v-if="pedido.status == 'waiting'">
                                <a href="javascript:;" class="btn cancelar"
                                   @click.prevent="deleteTracking([pedido.tracking_code])"> Cancelar Pagamento </a>
                            </template>
                        </template>
                        <template v-else>
                            <a href="javascript:;" @click.prevent="getCotations(pedido.id,i)" class="btn comprar">Calcular
                                Cotação</a>
                        </template>
                    </td>
                </tr>
        ...
        <div class="wpme_pagination_wrapper">
            <ul class="wpme_pagination">
                <li v-for="i in Math.ceil(total/perpage)" v-show="total > perpage" :class="{'active': i == page}">
                    <a href="javascript:;" @click.prevent="pagego(i)"
                       v-if="i < 2 && i != page || i < page+2 && i > page || i > page-2 && i < page  ||  i > (total / perpage)-1 && i != page || i == page">{{i}}</a>
                    <span class="ret"
                          v-show="(i == page-2 | i == page+1) && (Math.ceil(total / perpage) > 4) && Math.ceil(total /perpage) > i+1"> ...  </span>
                </li>
            </ul>
        </div>
        ...
        <div class="modal" v-show="show_confirm_modal">
            ...
            <a href="javascript:;" class="btn cancelar" @click.prevent="cancelTicket()">Quero cancelar</a> <a
                    href="javascript:;" @click.prevent="toogleConfirmer" class="btn fechar">Fechar</a>
        ...
        <div class="wpme_message" v-if="message.show_message">
            <div class="wpme_message_header"
                 :class="{'wpme_success': message.type == 'success', 'wpme_error': message.type == 'error'}">
                {{message.title}}
            </div>
            ...
            <div class="wpme_wrapper_center">
                <template v-if="payment_tracking_codes.length > 0 ">
                    <div class="wpme_message_comprar"><a href="javascript:;" @click.prevent="goDirectPay">Pagar</a>
                    </div>
                </template>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            openMultiplePaymentSelector: function () {
                this.payment_tracking_codes = [];
                if (this.pedidos_checked.length > 0 && this.pedidos_checked.find(function (data) {
                        return data == true;
                    })) {
            ...
            PrintMultiple: function () {
                var trackings = [];
                if (this.pedidos_checked.length < 1 || !this.pedidos_checked.find(function (data) {
                        return data == true;
                    })) {
